...
(``tests/gateway/fixtures/relay_wire_vector.json``) and the v2 gateway vector
(``gateway_wire_vector.json``), pinning the Python implementation against a
...
_FIXTURE_PATH = Path(__file__).resolve().parent / "fixtures" / "relay_wire_vector.json"
...
def test_base64_inputs_are_canonical():
    ...
    with pytest.raises(relay_e2ee.InvalidPrivateKeyError):
...
# ===========================================================================
# F. GATEWAY cross-language interop (production AAD helpers)
# ===========================================================================
#
# Pins the payload-vs-key-wrap AAD split across the language boundary. A test
# that round-trips one language against itself cannot catch a payload/key-wrap
# AAD divergence, and the relay fixture (relay_wire_vector.json) carries zero
# gateway labels. Here Python opens the committed gateway wire vector
# (gateway_wire_vector.json, regenerated and byte-verified in-tree by
...
def test_gateway_aad_labels_are_distinct_payload_vs_key(gateway_vector):
    """Payload AAD and key-wrap AAD must differ, so neither can be used in
    place of the other. Pin the split."""
...
def test_gateway_event_unwrap_with_payload_aad_fails_invalid_tag(gateway_vector):
    """Unwrapping the per-event key with the PAYLOAD aad (gatewayEvent) instead
    of the KEY aad (gatewayEventKey) must raise InvalidTag — pins that the key
    unwraps only under its own AAD, never the payload AAD."""
    from cryptography.exceptions import InvalidTag
