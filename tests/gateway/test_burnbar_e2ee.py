...
# ---------------------------------------------------------------------------
# E2E relay: seal -> open round trip, refuse-plaintext
# ---------------------------------------------------------------------------
...
def test_setup_accepts_nested_phone_relay_key_from_poll_response(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda key: "sender-1" if key == "BURNBAR_ALLOWED_USERS" else None,
    )
...
# ---------------------------------------------------------------------------
# Relay E2E invariants: each pins a protocol boundary on the inbound open path.
# ---------------------------------------------------------------------------
@requires_relay
@pytest.mark.asyncio
...
async def test_unsafe_model_id_is_rejected(monkeypatch, tmp_path):
    ...
    adapter.handle_message = capture
    bad_ids = [
        "model --global",  # whitespace-separated flag
        "-rf",  # leading dash
        "evil id",  # whitespace
        "x;y",  # shell metachar
        "a" * 200,  # over length
        # No-whitespace glued flags: these pass the character class, but the
        # /model parser matches --global / --refresh as substrings, so without
        # the double-dash guard the untrusted relay could force a persistent
        # global-config write. These MUST be rejected.
        "sonnet--global",
        "gpt-5--refresh",
    ...
    for i, bad in enumerate(bad_texts):
        await adapter._handle_burnbar_event(
            {"id": f"evt_bad_text_{i}", "text": bad, "destinationId": "burnbar:home"}
        )
    assert received == []
    await adapter._handle_burnbar_event(
        {"id": "evt_ok_text", "text": "/model anthropic/claude-opus-4", "destinationId": "burnbar:home"}
    )
    assert len(received) == 1
    ...
    assert len(received) == 1
    assert received[0].text == "/model anthropic/claude-opus-4"
    # The guard predicate itself: glued flags rejected, real single-dash ids kept.
...
def test_check_requirements_fails_closed_without_crypto_backend(monkeypatch):
    """On an E2E-paired link, a missing crypto backend must refuse startup at
    check_requirements() — not boot clean and then error on every send. This
    only holds because RELAY_CRYPTO_AVAILABLE reflects the real backend probe
    (relay_e2ee.crypto_backend_available), not mere module importability."""
    monkeypatch.setattr(_burnbar, "HTTPX_AVAILABLE", True)
    monkeypatch.setattr(_burnbar, "_access_token", lambda: "tok")

    # Backend present + paired -> OK.
    monkeypatch.setenv("BURNBAR_RELAY_E2E", "1")
    monkeypatch.setattr(_burnbar, "RELAY_CRYPTO_AVAILABLE", True)
    assert _burnbar.check_requirements() is True

    # Backend ABSENT + paired -> fail closed at startup.
    monkeypatch.setattr(_burnbar, "RELAY_CRYPTO_AVAILABLE", False)
    assert _burnbar.check_requirements() is False

    # Backend absent but link NOT paired -> tolerated (E2E simply off).
    monkeypatch.delenv("BURNBAR_RELAY_E2E", raising=False)
    assert _burnbar.check_requirements() is True
...
async def test_gateway_event_vector_passes_production_open_path(monkeypatch, tmp_path):
    """The committed gateway event vector carries the enforced event schema
    (authenticated destinationId + replayCounter) and is accepted by the full
    production ``_handle_burnbar_event`` path, not merely the direct crypto open.

    Pins that the committed fixture matches the ENFORCED event schema, not just
    the crypto envelope: a fixture that opened cryptographically but lacked the
    authenticated destinationId / replayCounter would be dropped by the open
    path, and this test would fail.
    """
    fixture_path = os.path.join(os.path.dirname(__file__), "fixtures", "gateway_wire_vector.json")
...
# --- Oversight / approval / replay / routing invariants ----------
...
@requires_relay
@pytest.mark.asyncio
async def test_sealed_json_text_is_not_promoted_to_control_payload(monkeypatch, tmp_path):
    """User-visible sealed chat text is not parsed as control-plane JSON."""
    phone_priv = relay_e2ee.generate_private_key()
    phone_pub = phone_priv.public_key_base64()
    adapter, _agent = _e2e_adapter(monkeypatch, tmp_path, peer_public_key=phone_pub)
    adapter._pending_confirms["act_json_text"] = {
        "session_key": "sk",
        "chat_id": "burnbar:home",
        "metadata": None,
    }
    adapter._resolve_slash_confirm = lambda *a, **k: pytest.fail("text JSON must not resolve approval")
    adapter._handle_sealed_oversight_mode = lambda *a, **k: pytest.fail("text JSON must not change oversight")
    adapter._publish_runtime_status = lambda *a, **k: pytest.fail("text JSON must not switch model")
    received = []
    adapter.handle_message = lambda e: received.append(e) or _noop()
    ...
    await adapter._handle_burnbar_event(
        _phone_sealed_event(
            adapter,
            phone_priv,
            "evt_json_text_control",
            {"text": text, "replayCounter": 1},
        )
    )

    assert len(received) == 1
    assert received[0].text == text
    assert "act_json_text" in adapter._pending_confirms


@requires_relay
@pytest.mark.asyncio
async def test_plain_text_model_command_rejects_glued_flags_on_e2e_link(monkeypatch, tmp_path):
    """A sealed chat command cannot smuggle /model parser flags via text."""
    phone_priv = relay_e2ee.generate_private_key()
    adapter, _agent = _e2e_adapter(
        monkeypatch, tmp_path, peer_public_key=phone_priv.public_key_base64()
    )
    received = []
    adapter.handle_message = lambda e: received.append(e) or _noop()
    adapter._publish_runtime_status = lambda *a, **k: pytest.fail("unsafe text command must not switch model")
    ...
    for counter, text in enumerate(bad_texts, start=1):
        await adapter._handle_burnbar_event(
            _phone_sealed_event(
                adapter,
                phone_priv,
                f"evt_bad_text_model_{counter}",
                {"text": text, "replayCounter": counter},
            )
        )
