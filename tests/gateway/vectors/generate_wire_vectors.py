...
the documented primitives in :mod:`gateway.crypto.relay_e2ee`. The same wire
...
def build_gateway_vector(spec: dict) -> dict:
    ...
    is the phone (B). Senders are bound per slot (phone→agent for event/modelSwitch,
    agent→phone for message/attachment), exactly mirroring the adapter's send paths.
    """
    ns = _ns()
    out = dict(spec)
