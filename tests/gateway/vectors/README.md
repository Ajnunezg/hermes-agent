...
key wrap (`gateway/crypto/relay_e2ee.py`).

| Fixture | Scheme |
| --- | --- |
| `../fixtures/HermesRelayWireVector.json` | v1 realtime relay (anonymous, single-DH) |
| `../fixtures/HermesGatewayWireVector.json` | v2 gateway (authenticated 2-DH) — event, agent reply, model_switch, attachment |
...
* **Two parties.** In the v2 gateway vector, `event.recipientPrivateKey` is the
  ...
  (phone→agent for event/model_switch, agent→phone for reply/attachment), mirroring
