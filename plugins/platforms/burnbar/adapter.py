...
try:
    ...
    # relay_e2ee imports fine even without `cryptography` (it imports the backend
    # lazily inside each function), so module-importability is NOT proof the
    # backend is present. Probe the real dependency so the startup fail-closed
    # check fires — and prints the install hint — in exactly the missing-backend
    # scenario it exists for, instead of booting clean then erroring on first send.
    RELAY_CRYPTO_AVAILABLE = relay_e2ee.crypto_backend_available()
except ImportError:  # pragma: no cover - exercised only when relay_e2ee itself is unimportable.
...
# Catalog-id charset for a model-switch value. First char alphanumeric so the
# value can never start with a dash, no whitespace, and (enforced separately in
# _is_safe_model_id) no ``--`` run. The double-dash guard is load-bearing: the
# ``/model`` slash parser matches ``--global`` / ``--refresh`` as *substrings*,
# so the charset alone would let a no-whitespace id like ``sonnet--global``
# smuggle a flag. ``--provider`` needs whitespace and is blocked by the charset.
_SAFE_MODEL_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._:/-]{0,179}$")
_MODEL_FLAG_DASH_RE = re.compile(r"[\u2012\u2013\u2014\u2015](provider|global|refresh)")
...
def _is_safe_relay_model_command(text: str) -> bool:
    """Reject relay-sourced ``/model`` text that could smuggle parser flags."""
    stripped = text.strip()
    if not stripped:
        return True
    ...
    if len(parts) == 1:
        return True
    raw_args = _MODEL_FLAG_DASH_RE.sub(r"--\1", parts[1].strip())
    if any(flag in raw_args for flag in ("--provider", "--global", "--refresh")):
        return False
    return _is_safe_model_id(raw_args)
...
def _gateway_aad(*parts: str) -> bytes:
    ...
    # RelayNamespace.aad so it never emits an unvalidated, collidable AAD.
...
class _RelaySealer:
    ...
    # NOTE: there is intentionally no standalone ``open_model_switch``. A sealed
    # model_switch arrives as an ordinary sealed gateway event ({"modelId": ...}
    # in the event payload) and is opened through ``open_event`` using the
    # gatewayEvent AAD labels. Chat text is never parsed for control JSON;
    # control payloads must be explicit authenticated fields in the sealed object.
...
class BurnBarAdapter(BasePlatformAdapter):
    ...
    def _pin_peer_public_key(
        ...
        *,
        ...
        an inbound doc are not authenticated. So a new pin (the very first peer key
        for this link) is only ever established from authenticated pairing data:
        in production, ``interactive_setup`` writes ``BURNBAR_RELAY_PEER_PUBLIC_KEY``
        after the human confirms the safety code, and ``__init__`` reads it. This
        method also exposes ``allow_new_pin=True`` as the explicit seed entry point
        for that pairing data (used by setup-style callers and tests). The live
    ...
    async def _handle_burnbar_event(self, raw: dict) -> None:
        ...
        if not text:
            return
        if not _is_safe_relay_model_command(text):
            logger.warning("[%s] dropped relay-sourced unsafe /model command %r", self.name, text)
            return
