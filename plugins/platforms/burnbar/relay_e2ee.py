...
binds the per-message id, but dedup across messages is a caller responsibility;
this module only authenticates and opens one envelope at a time.
...
mobile relay wire format. This module introduces only the v2 byte format
covered by the in-tree vectors.
...
# Published relay key version for the authenticated v2 key-wrap primitive. The
# anonymous v1 wrap/unwrap remains available only when callers omit the pinned
...
def crypto_backend_available() -> bool:
    """True only when the ``cryptography`` backend can actually be imported.

    This module imports ``cryptography`` lazily inside each function (so that
    importing the module never pulls the backend in), which means a successful
    ``import relay_e2ee`` does NOT prove the backend is installed. Callers can
    use this probe to test the real dependency rather than module importability.
    """
    try:  # pragma: no cover - the absent branch only runs without the extra
...
class InvalidPrivateKeyError(RelayCryptoError):
    """The relay private key is invalid (bad base64 or not a P-256 scalar)."""
...
class RelayNamespace:
    ...
    def aad(self, parts: list[str]) -> bytes:
        ...
        # it is a fail-closed guard for any caller that forwards a less constrained
        # string.
        for part in parts:
...
class RelayPrivateKey:
    ...
    raw_representation: bytes = field(repr=False)
    ...
    def from_base64(cls, raw_base64: str) -> "RelayPrivateKey":
        ...
        try:
            ...
            raise InvalidPrivateKeyError("relay private key is invalid") from exc
...
class AgentRelayIdentity:
    ...
    private_key: RelayPrivateKey = field(repr=False)
...
def unwrap_symmetric_key(
    ...
    **v2 (``sender_public_base64`` supplied)** is the authenticated 2-DH unwrap.
    ...
    symmetric_key = AESGCM(wrapping_key).decrypt(
        body[:_NONCE_BYTE_COUNT], body[_NONCE_BYTE_COUNT:], aad
    )
    if len(symmetric_key) != _SYMMETRIC_KEY_BYTE_COUNT:
        raise InvalidSymmetricKeyError("symmetric key must be 32 bytes")
