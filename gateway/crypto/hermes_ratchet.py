"""Hermes Gateway ratcheted message crypto.

This is the Python mirror of OpenBurnBar's Swift
``HermesRatchetCrypto`` type. It implements the same P-256 / HKDF-SHA256 /
AES-256-GCM ratchet state machine and the same length-prefixed AAD byte format:
...
The module is deliberately standalone today. The BurnBar adapter integration
points are documented in ``plugins/platforms/burnbar/HERMES_RATCHET_INTEGRATION.md``;
until that schema path exists, callers must not fall back to plaintext.
"""
...
class TooManySkippedKeysError(HermesRatchetError):
    """The sender skipped farther than the configured bound."""
...
def initiator_state(
    *,
    ...
    """Create the first sender state, mirroring Swift ``initiatorState``."""
...
def responder_state(
    *,
    ...
    """Create the first receiver state, mirroring Swift ``responderState``."""
    _validate_symmetric_key(shared_secret, "sharedSecret")
...
def decrypt(
    ...
    *,
    ...
    """Decrypt and advance ``state`` in place."""
