...
**Why did v2 not use RFC 9180 HPKE?** v2 is HPKE-AuthEncap-shaped, but it is not
HPKE wire framing. The project keeps this bespoke frame only because the Swift,
Kotlin, and Python clients already share byte-exact CryptoKit-compatible vectors
and gateway envelopes. The standard HPKE migration is v3, with a new
``relayKeyVersion`` and new vectors; do not silently mutate the v2 layout.
...
# RFC 9180 IDs: DHKEM(P-256, HKDF-SHA256), HKDF-SHA256, AES-256-GCM.
_HPKE_KEM_ID = 0x0010
_HPKE_KDF_ID = 0x0001
...
class RelayKeyWrapV3:
    """RFC 9180 HPKE Auth mode key-wrap envelope fields.

    ``enc`` is the HPKE encapsulated key (P-256 X9.63, base64). ``wrapped_key``
    is the HPKE ciphertext+tag over the 32-byte content key (base64). The payload
    and attachment AES-GCM layers remain the existing gateway layers.
    """
...
def _hpke_extract_and_expand(dh: bytes, kem_context: bytes) -> bytes:
    """RFC 9180 DHKEM ExtractAndExpand for P-256/HKDF-SHA256."""
...
def _hpke_auth_key_schedule(shared_secret: bytes, info: bytes) -> tuple[bytes, bytes]:
    """RFC 9180 Auth mode key schedule for single-shot AES-256-GCM seal/open."""
    psk_id_hash = _hpke_labeled_extract(
        _HPKE_SUITE_ID, b"", b"psk_id_hash", b""
    )
...
# key-schedule / labeled-KDF primitives above are RFC 9180 §4-§6; these two
